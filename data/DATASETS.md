...
| `dataset/elections_2` | `elections_2` | Second election batch — separate deploy or preview |
...
Dataset branches should stay aligned with `main` for app fixes and new analyses:

```bash
git checkout dataset/elections_2
git merge main
# Resolve conflicts only in src/ — never copy data/bets/ from main onto this branch
npm run build:data
```

If `data/bets/` conflicts during merge, keep this branch’s files and discard `main`’s
Excel imports.
