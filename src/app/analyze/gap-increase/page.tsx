...
export default async function GapIncreasePage({
  searchParams,
}: GapIncreasePageProps) {
  const params = await searchParams;
  const periodDays = parsePeriodDays(params.days);
  const { failures } = loadAllBets();
  const analysis = getGapIncreaseAnalysis(periodDays);
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-6 py-10">
      <section className="space-y-4">
        ...
        <p className="max-w-3xl text-lg text-zinc-600">
          When the gap between #1 and #2 widens for 2 consecutive days (daily
          closes), track whether the leader becomes the final winner.
        </p>
        <AnalysisNav active="gap-increase" />
