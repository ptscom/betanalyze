...
export default async function ConsecutiveDownPage({
  searchParams,
}: ConsecutiveDownPageProps) {
  ...
  const analysis = getConsecutiveDownDaysAnalysis(periodDays, streakLength);
    ...
    <main className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-8 px-6 py-10">
      <section className="space-y-4">
        ...
        <p className="max-w-3xl text-lg text-zinc-600">
          Find the first candidate whose daily close falls for 3 or 5 consecutive
          days in the window. Track whether that pick wins at close.
        </p>
        <AnalysisNav active="consecutive-down" />
